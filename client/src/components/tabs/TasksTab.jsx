...
function TaskInsights({ realTasks }) {
  ...
  // ── Chart 1: Completion Progress ───────────────────────────────────────
  // Single stacked bar: done / remaining / overdue
  ...
  const fmtUSD = (v) => v; // unused but keeps pattern consistent
          ...
          <div className="budget-chart-section">
            ...
            <ResponsiveContainer width="100%" height={52}>
              ...
              >
                ...
                <Bar dataKey="overdue"   stackId="a" fill="var(--red)"            fillOpacity={0.75}
                  radius={[0, 4, 4, 0]}>
                  <LabelList
                    dataKey="done"
                    position="right"
                    ...
                    >
                      ...
                      <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                        {priorityData.map((entry) => (
                          <Cell key={entry.name}
                            fill={PRIORITY_COLORS[entry.name] || "var(--accent-primary)"}
                            fillOpacity={0.8}
                          />
                        ))}
                        <LabelList
...
export function TasksTab({ state, updateData, appendAuditLog, isArchived, showToast, searchHighlight, clearSearchHighlight, setActiveTab, setSearchHighlight }) {
  ...
  const saveTasks = (next) => updateData("tasks", next);

  const handleAdd    = (t) => { if (isArchived) return; saveTasks([...tasks, t]); showToast("Task added"); setShowModal(false); setPrefilledTask(null); };
  const handleEdit   = (t) => { if (isArchived) return; saveTasks(tasks.map(x => x.id===t.id ? t : x)); showToast("Task updated"); setEditTask(null); };
  const handleDelete = (id) => { if (isArchived) return; saveTasks(tasks.filter(t => t.id!==id)); showToast("Task deleted"); setDeleteConfirm(null); };

  // Shared completion logic — used by toggleDone and handleLinkConfirm
  const completeTask = (t) => {
    saveTasks(tasks.map(x => x.id===t.id ? {...x, done:!t.done} : x));
    if (!t.done) appendAuditLog("Completed", `Completed task — ${t.task || "Task"}`);
    showToast(t.done ? "Task reopened" : "Task completed");
  };

  // Link confirm state: { task, action, label }
  // action: "markPaid" | "markPrepComplete"
  ...
  const toggleDone = (id) => {
    ...
    // Intercept qualifying completions that have a resolvable linked record
    if (!t.done && t.sourceId && t.sourceCollection) {
      if (t.sourceCollection === "expenses") {
        const exp = (state?.expenses || []).find(e => e.id === t.sourceId);
        if (exp && !exp.paid) {
      ...
      if (t.sourceCollection === "prep") {
        const item = (state?.prep || []).find(p => p.id === t.sourceId);
    ...
    // No qualifying link (or already resolved) — complete normally
    completeTask(t);
  };

  // Handle the user's choice in the link confirm modal
  const handleLinkConfirm = (withWriteBack) => {
    if (!linkConfirm) return;
    const { task, action } = linkConfirm;
    if (withWriteBack) {
      if (action === "markPaid") {
        const expenses = state?.expenses || [];
        updateData("expenses", expenses.map(e =>
          e.id === task.sourceId ? { ...e, paid: true } : e
        ));
        appendAuditLog("Updated", `Marked paid via task completion — ${task.task}`);
        showToast("Expense marked paid");
      }
      if (action === "markPrepComplete") {
        const prep = state?.prep || [];
        updateData("prep", prep.map(p =>
          p.id === task.sourceId ? { ...p, status: "Complete", progress: 100 } : p
        ));
        appendAuditLog("Updated", `Marked prep item complete via task completion — ${task.task}`);
  ...
  // Promote a suggestion into a new task via pre-filled modal
  ...
  // Dismiss a suggestion permanently (stored as a sentinel in tasks array)
  ...
  const handleRestoreSuggestions = () => {
    if (isArchived) return;
    saveTasks(tasks.filter(t => !t.dismissed));
  };

  // Compute suggestions from full state, excluding dismissed ones
  const dismissedIds  = new Set(tasks.filter(t => t.dismissed).map(t => t.id));
  const allSuggestions = computeSuggestions(state);
  ...
  const filtered = realTasks.filter(t => {
    ...
    // Always float done tasks to the bottom within any sort
  ...
  // Group by category when not searching/filtering and sort is "due" or "category"
  ...
  return (
    <div>
      ...
      {/* Link confirm — shown when completing a task linked to an expense or prep item */}
      {linkConfirm && (
