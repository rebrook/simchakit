...
export function FavorsTab({ state, updateData, setActiveTab, isArchived, showToast, searchHighlight, clearSearchHighlight, adminConfig }) {
  ...
  const [setupOpen,     setSetupOpen]     = useState(!favorConfig.givingFavors);
  ...
  const saveFavors = (next) => updateData("favors", { config: favorConfig, items: next });
  const saveConfig = (next) => updateData("favors", { config: next, items: favors });
  const setFC = (k, v) => { if (isArchived) return; saveConfig({ ...favorConfig, [k]: v }); };

  const handleAdd    = (f)  => { if (isArchived) return; saveFavors([...favors, f]);                              showToast("Favor added");   setShowModal(false); };
  const handleEdit   = (f)  => { if (isArchived) return; saveFavors(favors.map(x => x.id===f.id ? f : x));       showToast("Favor updated"); setEditFavor(null);  };
  const handleDelete = (id) => { if (isArchived) return; saveFavors(favors.filter(f => f.id!==id));               showToast("Favor deleted"); setDeleteConfirm(null); };

  const cyclePre = (id) => {
    if (isArchived) return;
    saveFavors(favors.map(f => {
      if (f.id !== id) return f;
      const next = { "TBD":"Yes", "Yes":"No", "No":"TBD" };
      return { ...f, preprint: next[f.preprint||"TBD"] || "TBD" };
    }));
  };


  // Add person from available panel
  const addFromPanel = (p) => {
    if (isArchived) return;
    const hhMap = Object.fromEntries(households.map(h => [h.id, h]));
    const name = (p.firstName || p.lastName)
      ? `${p.firstName||""} ${p.lastName||""}`.trim()
      : (p.name || "Unnamed");
    const sizeVal = favorConfig.sizeSource === "pant"   ? p.pantSize  || ""
  ...
  const removeFromList = (id) => { if (isArchived) return; saveFavors(favors.filter(f => f.id !== id)); showToast("Favor removed"); };

  // ── Derived data ───────────────────────────────────────────────────────────
  const hhMap = Object.fromEntries(households.map(h => [h.id, h]));
  const assignedPersonIds = new Set(favors.map(f => f.personId).filter(Boolean));

  const getAvailablePool = () => {
    let pool = people.filter(p => !assignedPersonIds.has(p.id));
    if (favorConfig.whoGets === "kids")    pool = pool.filter(p => p.isChild);
    if (favorConfig.whoGets === "adults")  pool = pool.filter(p => !p.isChild);
    return pool;
  };
  ...
  const groups = [...new Set(people.map(p => getPersonGroup(p)).filter(Boolean))].sort();
  ...
  const filtered = favors.filter(f => {
    ...
    if (filterAtt !== "all") {
      ...
      const secId = favorConfig.eventSectionId;
      if (secId) {
  ...
  const usedCategories = [...new Set(favors.map(f => f.category||"").filter(Boolean))].sort();
  ...
  const sectionId     = favorConfig.eventSectionId;
  ...
  const sizeCounts    = {};
  favors.forEach(f => { const s = f.size||""; if (s) sizeCounts[s] = (sizeCounts[s]||0)+1; });
  ...
  return (
    <div className="tab-content">
      ...
      <div className="section-header">
        <div>
          <div className="section-title">
            {favorConfig.favorDescription || "Favors"}
          </div>
          <div className="section-subtitle">
            {favorConfig.givingFavors
      ...
      <div className="card" style={{ marginBottom: 20 }}>
            ...
            {!setupOpen && (
              <div className="card-subtitle" style={{ marginBottom: 0, marginTop: 4 }}>
                {favorConfig.givingFavors
                  ? [
                      favorConfig.favorDescription || "Favors",
                      favorConfig.whoGets === "kids"   ? "Kids only"
                        : favorConfig.whoGets === "adults" ? "Adults only"
                        : favorConfig.whoGets === "manual" ? "Manual selection"
                        : "All guests",
                      favorConfig.needsSizing ? "Sizing on" : null,
                      favorConfig.isPersonalized ? "Personalized" : null,
                      favorConfig.eventSectionId ? `🎫 ${(timeline.find(e=>e.id===favorConfig.eventSectionId)||{}).title||"Sub-event linked"}` : null,
                    ].filter(Boolean).join(" · ")
        ...
        {setupOpen && (
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom: favorConfig.givingFavors ? 14 : 0 }}>
              <input type="checkbox" id="cfg-giving" checked={!!favorConfig.givingFavors}
            ...
            {favorConfig.givingFavors && (<>
              ...
              <div className="form-row" style={{ marginBottom:12 }}>
                ...
                <input className="form-input" value={favorConfig.favorDescription||""}
              ...
              <div className="form-row" style={{ marginBottom:12 }}>
                ...
                <select className="form-input" value={favorConfig.whoGets||"all"}
              ...
              <div style={{ display:"flex", gap:24, flexWrap:"wrap" }}>
                {[
                  ...
                  <label key={key} style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, color:"var(--text-primary)" }}>
                    <input type="checkbox" checked={!!favorConfig[key]}
              ...
              <div className="form-row" style={{ marginTop:12 }}>
                ...
                <select className="form-input" value={favorConfig.eventSectionId||""}
      ...
      {favorConfig.givingFavors && (<>
        ...
        {/* ── Size summary card ── */}
        {favorConfig.needsSizing && totalFavors > 0 && (
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                  <thead>
                    <tr style={{ borderBottom:"1px solid var(--border)" }}>
                      <th style={{ padding:"6px 10px", textAlign:"left",   fontWeight:700, color:"var(--text-muted)", fontSize:11, textTransform:"uppercase", letterSpacing:"0.04em", whiteSpace:"nowrap" }}>Name</th>
                      <th style={{ padding:"6px 10px", textAlign:"left",   fontWeight:700, color:"var(--text-muted)", fontSize:11, textTransform:"uppercase", letterSpacing:"0.04em", whiteSpace:"nowrap" }}>Category</th>
                      {favorConfig.needsSizing     && <th style={{ padding:"6px 10px", textAlign:"left",   fontWeight:700, color:"var(--text-muted)", fontSize:11, textTransform:"uppercase", letterSpacing:"0.04em", whiteSpace:"nowrap" }}>Size</th>}
                      {favorConfig.isPersonalized  && (
                      ...
                      {favorConfig.eventSectionId && (
                        <th style={{ padding:"6px 10px", textAlign:"center", fontWeight:700, color:"var(--text-muted)", fontSize:11, textTransform:"uppercase", letterSpacing:"0.04em", whiteSpace:"nowrap" }}>
                          Attending
                        </th>
                      )}
                      <th style={{ padding:"6px 10px", textAlign:"left",   fontWeight:700, color:"var(--text-muted)", fontSize:11, textTransform:"uppercase", letterSpacing:"0.04em", whiteSpace:"nowrap" }}>Notes</th>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      filtered.map(f => (
                        <tr key={f.id} id={`row-${f.id}`} style={{ borderBottom:"1px solid var(--border)" }}>
                          ...
                          {favorConfig.needsSizing && (
                            <td style={{ padding:"6px 10px", color:"var(--text-secondary)" }}>{f.size||"—"}</td>
                          )}
                          {favorConfig.isPersonalized && (
                            <td style={{ padding:"6px 10px", color:"var(--text-secondary)" }}>{f.printName||"—"}</td>
                          )}
                          {favorConfig.isPersonalized && (
                            <td style={{ padding:"6px 10px", textAlign:"center", cursor:"pointer", fontWeight:600, color:STATUS_STYLE[f.preprint||"TBD"] }}
                          ...
                          {favorConfig.eventSectionId && (() => {
                            ...
                            return (
                              <td style={{ padding:"6px 10px", textAlign:"center", fontWeight:600, color:STATUS_STYLE[label] }}>
                                {label}
                              </td>
                            );
                          })()}
                          <td style={{ padding:"6px 10px", color:"var(--text-muted)", fontSize:12, maxWidth:120, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{f.notes||"—"}</td>
      ...
      {showExport && (
        <FavorExportModal
          favors={favors} favorConfig={favorConfig}
