    ...
    <div className="modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal modal-lg"
        ...
        <div className="modal-body"
          style={{ overflowY:"auto", maxHeight:"calc(92vh - 80px)", paddingTop:16 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize:13, color:"rgba(255,255,255,0.85)", lineHeight:1.7 }}>
              SimchaKit is a real-time event planning dashboard for B'nei Mitzvot, weddings,
              and other simchas. Everything you need — guests, budget, vendors, tasks, seating,
              gifts, and a planning calendar — in one place, synced across all your devices.
            </div>
          </div>
              ...
              <div key={section.id} style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isCollapsed ? "var(--bg-surface)" : "var(--bg-subtle)",
                ...
                {!isCollapsed && (
                  <div style={{ padding:"16px 18px", background:"var(--bg-surface)" }}>
                    {section.items.map((item, i) => (
                      <div key={i} style={{
                        marginBottom: i < section.items.length - 1 ? 16 : 0,
          ...
          <div style={{
            ...
            SimchaKit · Brook Creative LLC · questions? check Admin Mode or server logs
          </div>
...
export function ActivityLogModal({ auditLog, onClear, isArchived, onClose }) {
              ...
              {["All", "Added", "Updated", "Deleted", "Completed"].map(a => (
                <button key={a} onClick={() => setFilterAction(a)}
                  style={{
                    ...
                    background: filterAction === a ? "var(--accent-primary)" : "var(--bg-subtle)",
          ...
          {showClearConfirm && (
            <div className="modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) { setShowClearConfirm(false); } }}>
              <div className="modal" style={{ maxWidth: 380 }} onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                  <div className="modal-title">Clear Activity Log?</div>
                  <button className="icon-btn" onClick={() => setShowClearConfirm(false)}>✕</button>
                </div>
                <div className="modal-body">
                  <p style={{ fontSize:14, color:"var(--text-secondary)", marginBottom:0 }}>
                    This will permanently wipe all activity log entries. This cannot be undone.
                  </p>
                </div>
                <div className="modal-footer">
                  <button className="btn btn-secondary" onClick={() => setShowClearConfirm(false)}>Cancel</button>
                  <button className="btn btn-danger" onClick={() => { onClear(); setShowClearConfirm(false); }}>
                    Clear Log
                  </button>
                </div>
              </div>
          ...
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "48px 24px", color: "var(--text-muted)", fontSize: 13 }}>
              ...
              <div style={{ fontSize: 12, lineHeight: 1.6 }}>
                Changes to guests, expenses, vendors, tasks, and gifts will appear here.
              </div>
            </div>
          ) : (
...
export function WhatsNewModal({ onClose }) {
  ...
  // Group changelog entries by major.minor
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button
              ...
              style={{
                ...
                background: showOpenOnly ? "var(--accent-primary)" : "var(--bg-subtle)",
                color: showOpenOnly ? "white" : "var(--text-muted)",
                fontFamily: "var(--font-body)",
                whiteSpace: "nowrap",
              }}
            >{showOpenOnly ? "Showing open only" : "Show open only"}</button>
            <button className="icon-btn" onClick={onClose}>✕</button>
          </div>
        </div>
        <div className="modal-body" style={{ overflowY: "auto", maxHeight: "calc(88vh - 80px)" }}>
          {error ? (
            <div className="alert alert-error">
              Could not load changelog. Make sure <code>changelog.json</code> exists in the server root.
            </div>
          ) : !data ? (
            ...
            <>
                    ...
                    <div key={group.key} style={{ marginBottom: gi < buildGroups(data.changelog).length - 1 ? 16 : 0 }}>
                      ...
                      <div
                        ...
                        style={{
                          ...
                          background: isFirst ? "var(--accent-light)" : "var(--bg-subtle)",
                          border: `1px solid ${isFirst ? "var(--accent-medium)" : "var(--border)"}`,
                          marginBottom: isCollapsed ? 0 : 12,
                        }}
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ fontSize: 13, fontWeight: 800, color: isFirst ? "var(--accent-primary)" : "var(--text-primary)" }}>
                            {group.key}
                          </span>
                          <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                            {group.entries.length} {group.entries.length === 1 ? "release" : "releases"} · {totalChanges} {totalChanges === 1 ? "change" : "changes"}
                          </span>
                        ...
                        <div style={{ paddingLeft: 4 }}>
                          {group.entries.map((entry, ei) => (
                            <div key={ei} style={{
                              ...
                              paddingBottom: ei < group.entries.length - 1 ? 16 : 0,
                              borderBottom: ei < group.entries.length - 1 ? "1px solid var(--border)" : "none",
                            }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                                <span style={{
                                  background: gi === 0 && ei === 0 ? "var(--accent-primary)" : "var(--bg-muted)",
                                  color: gi === 0 && ei === 0 ? "#fff" : "var(--text-secondary)",
              ...
              <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "20px 24px", marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {(data.roadmap || []).slice().filter(item => !showOpenOnly || item.status !== "complete").sort((a, b) => {
                    const sOrder = { planned: 0, future: 1, complete: 2 };
                    const pOrder = { P1: 0, P2: 1, P3: 2, P4: 3 };
                    const sa = sOrder[a.status] ?? 1;
                    const sb = sOrder[b.status] ?? 1;
                    ...
                    const priorityColors = {
                      P1: { bg: "var(--red-light)",    color: "var(--red)"          },
                      P2: { bg: "var(--gold-light)",   color: "var(--gold)"         },
                      P3: { bg: "var(--blue-light)",   color: "var(--blue)"         },
                      P4: { bg: "var(--bg-muted)",     color: "var(--text-muted)"   },
                    };
                    const pc = item.priority ? (priorityColors[item.priority] || priorityColors.P4) : null;
                    return (
                      <div key={i} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: item.detail ? 3 : 0 }}>
                            {item.version && <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-muted)", marginRight: 8 }}>{item.version}</span>}
                            {item.feature}
                          </div>
                        ...
                        <div style={{ display: "flex", gap: 5, flexShrink: 0, flexDirection: "column", alignItems: "flex-end" }}>
                          {pc && (
                            <span style={{
                              ...
                              background: pc.bg, color: pc.color, whiteSpace: "nowrap", letterSpacing: "0.04em",
                            }}>{item.priority}</span>
                          )}
                          <span style={{
                    ...
                    {data.knownIssues.filter(issue => !showOpenOnly || issue.severity !== "resolved").map((issue, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: "var(--gold-light)", color: "var(--gold)", whiteSpace: "nowrap" }}>
                ...
                <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", marginTop: 12 }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderBottom: auditCollapsed ? "none" : "1px solid var(--border)",
                    }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 20, background: "var(--blue-light)", color: "var(--blue)", whiteSpace: "nowrap" }}>
                        {showOpenOnly
                          ? (data.auditItems.filter(i => i.status !== "complete").length + " open")
                          : (data.auditItems.length + " items")}
                      </span>
                    </div>
                    <span style={{
                      fontSize: 13, color: "var(--text-muted)", display: "inline-block",
                      transition: "transform 0.2s", transform: auditCollapsed ? "rotate(-90deg)" : "rotate(0deg)",
                    }}>▾</span>
                  </div>
                          ...
                          <div key={i} style={{
                            ...
                            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                              ...
                              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
                                {pc && (
                                  <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: pc.bg, color: pc.color, whiteSpace: "nowrap" }}>
                                    {item.priority}
                                  </span>
                                )}
                                ...
                                {item.area && (
                                  <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 20, background: "var(--bg-muted)", color: "var(--text-muted)", whiteSpace: "nowrap" }}>
                                    {item.area}
                                  </span>
                                )}
                                {item.effort && (
              ...
              {data.versioning && (
                <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: "20px 24px", marginTop: 12 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 16 }}>
                    Scheme: <code style={{ background: "var(--bg-muted)", padding: "1px 6px", borderRadius: 4, fontSize: 12 }}>{data.versioning.scheme}</code>
                  </div>
                      ...
                      return (
                        <div key={i} style={{
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 160, flexShrink: 0 }}>
                            <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 20, background: c.bg, color: c.color, whiteSpace: "nowrap" }}>
                              {rule.level}
                            </span>
                  ...
                  {data.versioning.thresholds && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      ...
                      {data.versioning.thresholds.next_minor && (
                        <div style={{ padding: "8px 14px", background: "var(--blue-light)", borderRadius: "var(--radius-sm)", fontSize: 13, color: "var(--blue)", fontWeight: 600 }}>
                          📌 {data.versioning.thresholds.next_minor}
                        </div>
                      )}
                      {data.versioning.thresholds.next_major && (
                        <div style={{ padding: "8px 14px", background: "var(--accent-light)", borderRadius: "var(--radius-sm)", fontSize: 13, color: "var(--accent-primary)", fontWeight: 600 }}>
                          🚀 {data.versioning.thresholds.next_major}
                        </div>
                      )}
                    </div>
