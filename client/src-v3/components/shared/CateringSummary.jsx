        ...
        <div style={{ marginTop:16 }}>
                  ...
                  <tbody>
                    {[...new Set([...meals,...Object.keys(mealCounts)])].map((m,i,arr) => (
                      <tr key={m} style={{ borderBottom:i<arr.length-1?"1px solid var(--border)":"none" }}>
                        ...
                        <td style={{ padding:"8px 12px", textAlign:"right", fontWeight:700, color:(mealCounts[m]||0)>0?"var(--text-primary)":"var(--text-muted)" }}>{mealCounts[m]||0}</td>
                        <td style={{ padding:"8px 12px", textAlign:"right", color:"var(--text-muted)" }}>{mealTotals[m]||0}</td>
                      </tr>
                    ))}
                    {noMealChoice > 0 && (
                      <tr style={{ borderTop:"1px solid var(--border)", background:"var(--bg-subtle)" }}>
                        ...
                        <td style={{ padding:"8px 12px", textAlign:"right", color:"var(--orange)", fontWeight:700 }}>{noMealChoice}</td>
          ...
          {style !== "buffet-headcount" && (
            <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:16, padding:"10px 14px", background:"var(--bg-subtle)", borderRadius:"var(--radius-md)", border:"1px solid var(--border)" }}>
              ...
              <span style={{ fontSize:18, fontWeight:800, color:"var(--accent-primary)", fontFamily:"var(--font-display)" }}>{kosherConfirmed}</span>
              {kosherTotal !== kosherConfirmed && (
                <span style={{ fontSize:12, color:"var(--text-muted)" }}>({kosherTotal} total invited · {kosherTotal-kosherConfirmed} RSVP pending)</span>
              )}
            </div>
          )}
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                {dietaryPeople.map(p => {
                  ...
                  return (
                    <div key={p.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 12px", borderRadius:"var(--radius-sm)", background:confirmed?"var(--bg-surface)":"var(--bg-subtle)", border:"1px solid var(--border)", fontSize:13, flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", justifyContent:"flex-end" }}>
            <button className="btn btn-secondary btn-sm" onClick={handleCopy}>{copyMsg || "📋 Copy for caterer"}</button>
          </div>
        </div>
      )}
    </div>
