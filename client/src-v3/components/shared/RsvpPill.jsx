...
export function RsvpPill({ hh, open, onOpen, onSelect, statusStyle }) {
  const sc = statusStyle[hh.rsvpStatus] || statusStyle["Invited"];
  const dotColors = {
    "Invited":  "var(--blue)",
    "RSVP Yes": "var(--green)",
    "RSVP No":  "var(--red)",
    ...
    <div style={{position:"relative",display:"inline-block"}}>
      ...
      {open && (
        <div className="rsvp-dropdown" onClick={e=>e.stopPropagation()}>
          {RSVP_STATUSES.map(s => (
            <button key={s} className={`rsvp-dropdown-item ${hh.rsvpStatus===s?"active":""}`}
              onClick={()=>onSelect(s)}>
              <div className="rsvp-dot" style={{background: dotColors[s]||"var(--text-muted)"}} />
              {s}
              {hh.rsvpStatus===s && <span style={{marginLeft:"auto",fontSize:12}}>✓</span>}
            </button>
          ))}
        </div>
      )}
