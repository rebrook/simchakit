...
![Version](https://img.shields.io/badge/version-4.28.1-blue)
...
- **Sub-Event Support** — track attendance across multiple events (service, kiddush, reception); optional per-sub-event RSVP status with row-level chips and dedicated filtering on the Guests tab, and a per-sub-event "invite all by default" setting in Admin Mode for events where only a handful of households attend a given sub-event
